...
def simulate_actuator_rotation():
    ...
    and runs a simulation with visualization.
    """
    try:
        ...
        bugs = {
            "rolly1": [model.actuator("rolly1_L").id, model.actuator("rolly1_R").id, model.site("imu1").id, "imu1_accel", "imu1_gyro", "0.5"],
            "rolly2": [model.actuator("rolly2_L").id, model.actuator("rolly2_R").id, model.site("imu2").id, "imu2_accel", "imu2_gyro", "0.5"],
            "rolly3": [model.actuator("rolly3_L").id, model.actuator("rolly3_R").id, model.site("imu3").id, "imu3_accel", "imu3_gyro", "0.5"],
        }
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            ...
            # Optional: Adjust initial camera distance, azimuth, elevation
            ...
            print("\nMuJoCo Simulation Started. Press ESC or close window to exit.")
            ...
            while viewer.is_running():
                ...
                # Ensure consistent simulation time step
...
if __name__ == "__main__":
    # Call the simulation function when the script is executed
